...
void crearHilos(){
    ...
    for(i = 0; i < numeroHilos; i++){
        rc = pthread_create(&hilos[i], NULL, imprimirHilos, (void *)i);
...
void obtenerArgumentos(int cantidad, char* argumentos[]){
    ...
    for(i = 1; i < cantidad; i++){
        ...
        if(strcmp(argumentoAnterior,"-n") == 0 && strcmp(argumentoActual,"-w") != 0
           && strcmp(argumentoActual,"-p") != 0 && strcmp(argumentoActual,"-t") != 0){
            ...
            numeroHilos = atoi(argumentoActual);
    ...
    if(numeroHilos == 0 || puerto == 0 || path == "" || tipoProtocolo == ""){
...
void iniciarServer(){
    ...
    //Crear PreThreads
    crearHilos();
    int servidorActivo = 1;

    while (1) {
        if(hilosUsados > numeroHilos){
            printf("Se ha sobrepasado el número de clientes que pueden ser atendidos!!\n");
            break;
        }
        ...
        if (cliente < 0) {
            ...
            //write(cliente, respuesta, sizeof(respuesta) - 1);
